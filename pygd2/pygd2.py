...
class PyGd2(object):
    def __init__(self):
        ...
        self.cursor.execute('CREATE TABLE IF NOT EXISTS players(name TEXT PRIMARY KEY, id INTEGER)')

    def update_players(self, date):
        # update the database players, only run every so often
        for pname, pid in self._get_players(date).items():
            self._upsert_player(pname, pid)
            time.sleep(2)
        self.db.commit()

    def _get_players(self, today):
        ...
        for link in soup.find_all('a'):
            ...
            if href.startswith("gid_"):
                ...
                print(xml)
                players.update(self._process_players_xml(xml))
            time.sleep(2)

        # return as dictionary
        return players

    def _process_players_xml(self, url):
        players = {} # to be filled w/ player_name -> player_id mappings
    ...
    def _upsert_player(self, pname, pid):
    ...
    def _process_data(self, data, year, stats):
        buf = {}
        res = {}
    ...
    def get_stats(self, player_name, year, stats):
        result = {}
        player_id = self._get_player_id(player_name.lower())
        if not player_id:
            print("Could not find player id.")
            return result
        raw_h, raw_p = self._get_player_stats(player_id, year)
        ...
        if 'row' in data_h.keys():
            season_h = self._process_data(data_h['row'], year, stats)
        else:
            season_h = {}
        if 'row' in data_p.keys():
            season_p = self._process_data(data_p['row'], year, stats)
        else:
            season_p = {}
    ...
    def _get_player_stats(self, player_id, year):
